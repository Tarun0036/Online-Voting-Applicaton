	...
	<div id="container">
		...
		<div id="innerCont">
			<table>
				...
				<%
				    List<CandidateList> cand= (List)request.getAttribute("listOfCandies");
				    for (CandidateList c : cand) {
				%>
				<tr>
					<th><%=c.getcName()%></th>
					<th><%=c.getcEmail() %></th>
					...
					<th><%=c.getGender() %></th>
					<th><%=c.getcPhone() %></th>
					<th><%=c.getAdhaarNum() %></th>
					<th><%=c.getPanNum() %></th>
					<th><%=c.getMonthlyIncome() %></th>
					<th><%=c.getYearlyIncome() %></th>
					<th><%=c.getTotalProperty() %></th>
					<th><%=c.getCriminalBackground() %></th>
					<th style="color: red"><%=c.getVotes() %></th>
				</tr>
				<%
				}
				%>
